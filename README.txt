...
# Output						#
# ===================================================== #
sl1p produces a series of output files, which are explained within. Please note that adjusting some of the commandline options (for e.g. choosing a taxa database other than gg2011) will slightly change these filenames:
...
align_gg2011_filtered.tar.gz -> tarball of the output of QIIME's filter_alignment.py
align_gg2011.tar.gz -> output of QIIME's align_seqs.py
gg2011_97_prune.tre -> phylogeny created by sl1p by pruning the Greengenes 2011 phylogeny to those nodes in the dataset
library_stats_rdp-training_gg2011_n1_noRoot.txt -> output of biom summarize-table -i otu_table_rdp-training_gg2011_n1_noRoot.biom
library_stats_rdp-training_gg2011.txt -> output of biom summarize-table -i otu_table_rdp-training_gg2011.biom
otus_aux.tar.gz -> supplemental OTU tables including the removal of doubletons (n2), Root (noRoot), and singletons (n1)
otu_table_rdp-training_gg2011[.biom|.txt] -> output of QIIME's make_otu_table.py; chimeras removed (unless -m n used)
otu_table_rdp-training_gg2011_n1_noRoot[.biom|.txt] -> output of QIIME's make_otu_table.py; any OTUs assigned to Root/Unclassified removed using removeRoot.pl; singletons removed with QIIME's filter_otus_from_otu_table.py; chimeras removed (unless -m n used)
otu_table_rdp-training_gg2011_n1_noRoot_relAbund.txt -> OTU table with counts converted into relative abundance estimates per sample via sl1p_analysis.Rmd
phylo_aux.tar.gz -> supplemental phylogeny files including the default QIIME phylogeny (make_phylogeny.py)
qiime_params.txt -> QIIME's parameter file; supplied incase the user wishes to extend the analyses conducted here
rdp-training_gg2011_assigned_taxonomy_tr.tar.gz -> output of QIIME's assign_taxonomy.py
rep_set_aux.tar.gz -> output of OTU picking by AbundantOTU+ (unless -p is used to alter the OTU picking algorithm)
seqs_gg2011_otus.txt.tar.gz -> rep_set files from above formatted for input into QIIMe using abundantOTU_to_qiime.pl (unless -p is used to alter the OTU picking algorithm)
sl1p_analysis.Rmd -> an R markdown file of the preliminary analyses of the output data conducted in R; meant to be a starting place for the user to expand their analyses beyond what sl1p performs
sl1p_analysis.html -> output of sl1p_analysis.Rmd
wf_arare_rdp-training_gg2011 -> output of QIIME's alpha_rarefaction.py with otu_table_rdp-training_gg2011.biom as input
wf_arare_rdp-training_gg2011_n1_noRoot -> output of QIIME's alpha_rarefaction.py with otu_table_rdp-training_gg2011_n1_noRoot.biom as input
wf_beta_rdp-training_gg2011_e### -> output of QIIME's beta_diversity_through_plots.py with otu_table_rdp-training_gg2011.biom as input; the e-value indicated in the folder name is that recommended by QIIME (see library_stats_rdp-training__gg2011.txt for more information)
wf_beta_rdp-training_gg2011_e###_n1_noRoot -> output of QIIME's beta_diversity_through_plots.py with otu_table_rdp-training_gg2011_n1_noRoot.biom as input; the e-value indicated in the folder name is that recommended by QIIME (see library_stats_rdp-training_gg2011_n1_noRoot.txt for more information)
wf_taxa_summary_rdp-training_gg2011 -> output of QIIME's summarize_taxa_through_plots.py with otu_table_rdp-training_gg2011.biom as input
wf_taxa_summary_rdp-training_gg2011_n1_noRoot -> output of QIIME's summarize_taxa_through_plots.py with otu_table_rdp-training_gg2011_n1_noRoot.biom as input
